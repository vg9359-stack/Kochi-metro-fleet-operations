      ...
      {optimizationResult ? (
              ...
              <tbody className="divide-y divide-slate-800/60 bg-slate-900/40">
                {optimizationResult.assignments.map((item) => (
                  <tr key={item.trainId} className="hover:bg-slate-800/50 transition">
                    <td className="p-3 font-extrabold text-white">Bay #{item.recommendedBay}</td>
                    <td className="p-3 font-bold text-teal-400">{item.trainId}</td>
                    <td className="p-3 text-slate-400">Bay #{item.currentBay}</td>
                    <td className="p-3">
                      <span className="bg-slate-800 border border-slate-700 text-[10px] px-2 py-0.5 rounded font-mono">
                        {item.zone}
                      </span>
                    </td>
                    ...
                    <td className="p-3 text-right">
                      {item.requiresShunting ? (
                        ...
                        <span className="bg-teal-500/20 text-teal-400 border border-teal-500/30 text-[10px] font-bold px-2 py-0.5 rounded">
                          Optimal Stationed
                        </span>
                      )}
                    </td>
        ...
        <div className="bg-slate-950 border border-dashed border-slate-800 rounded-xl p-8 text-center space-y-2">
          <p className="text-xs text-slate-400">
            No optimization simulation currently active. Click above to auto-assign all 25 fleet units to depot tracks.
          </p>
        </div>
      )}
    </div>
